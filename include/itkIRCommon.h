...
inline static double OverlapPercent(image_t::SizeType size, const vec2d_t offset)
{
...
// 
xyz_t hsv_to_rgb(const xyz_t & HSV);
...
// spaces. This is used for colormapping.
// 
xyz_t rgb_to_hsv(const xyz_t & RGB);
...
typename IMG::Pointer
	...
	const typename IMG::PixelType & fill_value =
	itk::NumericTraits<typename IMG::PixelType>::Zero)
{
...
typename IMG::Pointer
	...
	typename IMG::PixelType fill_value =
	itk::NumericTraits<typename IMG::PixelType>::Zero)
{
...
typename IMG::Pointer
	...
	itk::NumericTraits<typename IMG::PixelType>::Zero)
{
	typename IMG::Pointer image =
		make_image<IMG>(width, height, spacing, fill_value);
...
public:
	...
	static std::vector<image_pointer_t> PruneInvalidTiles(const std::vector<bool> & omit,
	...
	static std::vector<unsigned int> GetSortedIndicies(// mosaic space tile bounding boxes:
...
/*
	...
	template<class pnt_t>
	static void GetEligibleIndiciesForSortedColumn(const pnt_t point,
	...
	//----------------
	template<class pnt_t>
	static std::vector<unsigned int> GetSortedTilesImagesForColumn(const pnt_t point,
																	 // mosaic space tile bounding boxes:
																	 const std::vector<pnt_t> MIN,
																     const std::vector<pnt_t> MAX)
	{
...
template<class T>
std::vector<typename itk::ImageRegionConstIteratorWithIndex<T> > DivideROIAmongThreads(typename T::ConstPointer fi, typename T::RegionType roi, int num_Threads=boost::thread::hardware_concurrency())
{
	typedef typename itk::ImageRegionConstIteratorWithIndex<T> itex_t;
...
//Divvy up the fixed image roi for each thread to work over...
  ...
  index_t fi_min = roi.GetIndex();
	...
	for(int iThread = 1; iThread < num_Threads; iThread++)
	{
		...
		//cout << iThread << " fi start index, " << "X0: " << listThreadStartIndex[iThread][0] << " Y0: " << listThreadStartIndex[iThread][1] << endl; 
	}
	...
	for(int iThread = 0; iThread < num_Threads; iThread++)
	{
		...
		//cout << iThread << " fixed roi, " << "X0: " << thread_min_index[0] << " Y0: " << thread_min_index[1] << " W: " << roi_size[0] << " H: " << roi_size[1] << endl; 
...
template<class T>
std::vector<typename itk::ImageRegionIteratorWithIndex<T> > DivideROIAmongThreads(typename T::Pointer fi, typename T::RegionType roi, int num_Threads=boost::thread::hardware_concurrency())
{
	...
	const mask_t * mi_mask = NULL)
{
	...
	{
		cerr << "image metric threw an exception:" << endl
			<< exception << endl;
                     ...
                     int num_Threads=boost::thread::hardware_concurrency())
{
		...
		16))
	{
		...
		if(!fi->TransformPhysicalPointToIndex(mi_max, fi_roi_max))
		{
			...
			//cout << "my_metric_mt: " << "cannot transform upper index x: " << fi_roi_max[0] << " y: " << fi_roi_max[1]  << endl;
			//cout << "my_metric_mt: " << "                 lower index x: " << fi_roi_min[0] << " y: " << fi_roi_min[1]  << endl; 
			//cout << "my_metric_mt: " << "                 fi size     w: " << fi_size[0] <<    " h: " << fi_size[1]  << endl; 
		}
	...
	*/
	...
	for(int iThread = 0; iThread < (int)list_fi_thread_itex.size(); iThread++)
	{
		...
		for (itex.GoToBegin(); !itex.IsAtEnd(); ++itex)
		{
			...
			if(uv[0] <= std::numeric_limits<double>::min() &&
				uv[1] <= std::numeric_limits<double>::min() )
			{
				continue;
			}
	...
	const double max_overlap = 1.0)
{
	...
	double m = my_metric_mt<TImage, interpolator_t>(overlap_area,
		fi,
		mi,
		fi_to_mi,
		fi_mask,
					...
					const unsigned int np = 15)
{
	...
	// FIXME:
	calc_mosaic_bboxes<point_t, transform_t>(transform,
		image_min,
		image_max,
		mosaic_min,
		mosaic_max,
								...
								const unsigned int np = 15)
{
	...
	calc_image_bboxes_load_images<IMG>(fn_image, 
		image_min, 
		image_max,
		shrink_factor,
		pixel_spacing);
	...
	bool dont_allocate = false)
{
	...
	for (ix[1] = origin[1]; ix[1] < y_end; ++ix[1])
	{
		...
		std::vector<unsigned int> PotentialTiles = AssembleUtil::GetSortedTilesImagesForColumn<point_t>(pointColumn, MIN, MAX); 
		if(PotentialTiles.size() <= 0)
		{
			continue; 
		}
...
public:
	...
	void execute(the_thread_interface_t * thread)
	{
		...
		for (ix[1] = origin[1]; ix[1] < y_end; ++ix[1])
		{
			...
			//std::vector<unsigned int> PotentialTiles = AssembleUtil::GetSortedTilesImagesForColumn<pnt_t>(pointColumn, min_, max_);
				...
				ix[0] += thread_stride_)
			{
				...
				for (unsigned int iK = RowStartTile; iK <= iEndTile; iK++)
				{
					...
					// don't try to add missing or omitted images to the mosaic:
					//James A: Profiler showed that omit_[k] was very expensive, so we check outside the loop once and only use valid indicies
					//if (InvalidTiles[k])
					//{
					//  continue;
					//}
					...
					{
						pxl_t pixel_weight = calc_pixel_weight(bbox_min_[k],
							bbox_max_[k],
							pt_k);
...
// 
template <class IMG, class transform_t, class img_interpolator_t>
typename IMG::Pointer make_mosaic_mt( unsigned int num_threads,
	bool assemble_mosaic_mask,
	mask_t::Pointer & mosaic_mask,
	const typename IMG::SpacingType & mosaic_sp,
	const typename IMG::PointType & mosaic_min,
	...
	bool dont_allocate = false)
{
  typedef typename IMG::PixelType pixel_t;
	...
	if (num_threads == 1)
	{
		...
		return make_mosaic_st<IMG, transform_t, img_interpolator_t>
	...
	calc_mosaic_bboxes<pnt_t, transform_t>(transform,
		bbox_min,
		bbox_max,
		MIN,
		MAX);
	...
	for (unsigned int i = 0; i < num_threads; i++)
	{
		assemble_mosaic_t<IMG, transform_t, img_interpolator_t> * t =
			new assemble_mosaic_t<IMG, transform_t, img_interpolator_t>
			(i,
			num_threads,
			mosaic,
			mosaic_mask,
        ...
        const int num_threads = boost::thread::hardware_concurrency())
{
	...
	return
	   make_mosaic_mt<IMG, transform_t, img_interpolator_t>(num_threads,
											assemble_mosaic_mask,
											mosaic_mask,
											mosaic_sp,
											mosaic_min,
	...
	bool dont_allocate = false)
{
	...
	return make_mosaic<IMG, transform_t, img_interpolator_t>
		(mosaic_sp,
		mosaic_min,
		mosaic_sz,
		num_images,
	...
	bool dont_allocate = false)
{
	...
	calc_mosaic_bbox<IMG, transform_t>(transform,
											image,
											mosaic_min,
											mosaic_max);
...
template <class IMG, class transform_t, class img_interpolator_t>
typename IMG::Pointer
	make_mosaic(const std::vector<typename IMG::ConstPointer> & image,
	const std::vector<typename transform_t::ConstPointer> & transform,
	const feathering_t feathering = FEATHER_NONE_E,
	...
	const std::vector<typename mask_t::ConstPointer> & image_mask = std::vector<typename mask_t::ConstPointer>(0))
{
	...
	return
		make_mosaic<IMG, transform_t, img_interpolator_t>
		(image[0]->GetSpacing(),
		num_images,
		omit_vec,
		transform,
...
public:
	save_mosaic_t(const typename T::ConstPointer image, 
...
//			cout << "saving " << m_filename.text() << endl;
		...
		{
			cerr << "Error saving " << m_filename.text() << endl
	  				  << exception << endl;
		}
	}
...
public:
	...
	template <typename T> 
	static void async(typename  T::ConstPointer image, 
		const the_text_t filename,
...
public:
	...
	assemble_mosaic_transaction_t(	int num_threads, //Number of threads to use for this transaction, sometimes we may have fewer tiles than threads
		bool assemble_mosaic_mask,
		typename mask_t::Pointer & mosaic_mask,
		const typename IMG::SpacingType & mosaic_sp,
		const typename IMG::PointType & mosaic_min,
		...
		typename IMG::PixelType background = 255.0,
		OUTPUT *output = NULL,
		the_text_t & saveFileName = ""): //If we should save the image to disk, use this filename
	...
	void execute(the_thread_interface_t * thread)
	{
		...
		if(m_num_threads == 1)
		{
			mosaic = make_mosaic_st<IMG, transform_t, itk::LinearInterpolateImageFunction<IMG, double> >
		...
		{
			mosaic = make_mosaic_mt<IMG, transform_t, itk::LinearInterpolateImageFunction<IMG, double> >
				(m_num_threads, 
				m_assemble_mosaic_mask,
				m_mosaic_mask,
				m_mosaic_sp,
...
public:
	...
	void execute(the_thread_interface_t * thread)
	{
		...
		std::vector<bool> InvalidTiles(num_tiles_, false);
		//std::vector<mask_t::ConstPointer> ValidTilesIndicies = AssembleUtil::PruneInvalidTiles<mask_t::ConstPointer>(omit_, mask_);
	...
	const std::vector<mask_t::ConstPointer> & image_mask)
{
	...
	{
		// use the old single-threaded code:
		return make_mask_st<transform_t>(mosaic_sp,
			num_images,
			omit,
			transform,
			image_mask);
	...
	for (unsigned int i = 0; i < num_threads; i++)
	{
		assemble_mask_t<transform_t> * t =
			new assemble_mask_t<transform_t>(i,
			num_threads,
			mosaic,
			num_images,
			omit,
...
{
	return make_mask_mt<transform_t>(num_threads,
		mosaic_sp,
		num_images,
		omit,
		transform,
                  ...
                  unsigned int num_threads = boost::thread::hardware_concurrency())
{
	...
	return make_mask<transform_t>(image_mask[0]->GetSpacing(),
		num_images,
...
// 
template <typename T>
typename T::Pointer warp(typename T::ConstPointer a,
						 base_transform_t::ConstPointer t)
{
	...
	return make_mosaic<T, base_transform_t, itk::NearestNeighborInterpolateImageFunction<T, double> >(image,
													 transform,
													 FEATHER_NONE_E);
}
					...
					const feathering_t feathering = FEATHER_NONE_E)
{
	...
	for (unsigned int i = 0; i < 3; i++)
	{
		...
		mosaic[i] =
			make_mosaic<IMG, transform_t, typename itk::LinearInterpolateImageFunction<IMG, double> >
				(mosaic_sp,
				mosaic_min,
				mosaic_sz,
				num_images,
...
// 
template <class IMG, class transform_t>
void make_mosaic_rgb(typename IMG::Pointer * mosaic,
					const xyz_t background_color,
					const std::vector<xyz_t> color,
					const std::vector<bool> & omit,
					const std::vector<typename transform_t::ConstPointer> & transform,
					...
					const feathering_t feathering = FEATHER_NONE_E)
	{
		...
		typename IMG::PointType mosaic_min;
		typename IMG::PointType mosaic_max;
		calc_mosaic_bbox<IMG, transform_t>
			(transform,
			image,
			mosaic_min,
			mosaic_max);
	...
	bool blab = true)
{
	...
	make_mosaic_rgb<T, base_transform_t>(mosaic,
	...
	const the_text_t & image_path)
{
	...
	for (std::list<the_text_t>::iterator iter = images.begin();
	...
	const mask_t * mask = NULL)
{
	...
	if (max_slope != 0.0)
	{
		double clipping_height =
			(double(nx * ny) / double(bins)) * max_slope;
	...
	if (shift_x != 0)
	{
		...
		for (int ty = 0; ty < ny; ty++)
		{
			...
			if (pixel_in_mask<T>(in, mask, ix))
			{
				...
				unsigned int bin = (unsigned int)((double(a - p_min) /
					double(p_rng)) *
					double(bins - 1));
				pdf[bin]++;
			}
			...
			if (pixel_in_mask<T>(in, mask, ix))
			{
				...
				unsigned int bin = (unsigned int)((double(d - p_min) /
					double(p_rng)) *
					double(bins - 1));
				pdf[bin]--;
			}
	...
	bool & flip)
{
	while (true)
	{
		...
		{
			cerr << "WARNING: unknown token: '" << token << "', ignoring ..."
				<< endl;
		}
	}
}
...
inline static void set_expected_major_progress(double ExpectedMajorTotal = 100)
{
...
inline static void set_expected_minor_progress(double ExpectedMinorTotal = 100)
{
...
inline static void increment_major_progress(double amount = 1)
{
...
inline static void increment_minor_progress(double amount = 1)
{
