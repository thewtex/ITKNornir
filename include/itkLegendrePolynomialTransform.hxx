...
LegendrePolynomialTransform<TScalarType, N>::LegendrePolynomialTransform()
{
...
LegendrePolynomialTransform<TScalarType, N>::TransformPoint(const InputPointType & x) const
{
...
LegendrePolynomialTransform<TScalarType, N>::BackTransformPoint(const OutputPointType & y) const
{
...
template <class TScalarType, unsigned int N>
void
LegendrePolynomialTransform<TScalarType, N>::ComputeJacobianWithRespectToParameters(const InputPointType & x,
...
template <class TScalarType, unsigned int N>
void
LegendrePolynomialTransform<TScalarType, N>::eval(const std::vector<ScalarType> &        x,
...
template <class TScalarType, unsigned int N>
void
LegendrePolynomialTransform<TScalarType, N>::setup_linear_system(const unsigned int                   start_with_degree,
                                                                 ...
                                                                 vnl_vector<double> &                 by) const
{
  ...
  const unsigned int offset = index_a(0, start_with_degree);

  const unsigned int extent = index_a(0, start_with_degree + degrees_covered) - offset;
  ...
  for (unsigned int row = 0; row < num_points; row++)
  {
    ...
    for (unsigned int i = start_with_degree; i < start_with_degree + degrees_covered; i++)
    {
      for (unsigned int j = 0; j <= i; j++)
      {
        const unsigned int k = i - j;
...
template <class TScalarType, unsigned int N>
void
LegendrePolynomialTransform<TScalarType, N>::solve_for_parameters(const unsigned int                  start_with_degree,
                                                                  const unsigned int                  degrees_covered,
                                                                  const std::vector<InputPointType> & uv,
                                                                  const std::vector<OutputPointType> & xy,
...
template <class TScalarType, unsigned int N>
void
LegendrePolynomialTransform<TScalarType, N>::PrintSelf(std::ostream & os, Indent indent) const
{
  ...
  for (unsigned int i = 0; i <= N; i++)
  {
    ...
    {
      unsigned int k = i - j;
      os << indent << "a(" << j << ", " << k << ") = " << a(j, k) << std::endl;
    }
  }
  for (unsigned int i = 0; i <= N; i++)
  {
    ...
    {
      unsigned int k = i - j;
      os << indent << "b(" << j << ", " << k << ") = " << b(j, k) << std::endl;
    }
  }
