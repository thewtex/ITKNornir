...
#ifndef _itkImageMosaicVarianceMetric_hxx
...
void
ImageMosaicVarianceMetric<TImage, TInterpolator>::setup_param_map(const std::vector<bool> & param_shared,
                                                                  const std::vector<bool> & param_active)
{
  ...
  assert(n_params > 0 && n_params == param_active.size() && n_params == transform_[0]->GetNumberOfParameters());
...
ImageMosaicVarianceMetric<TImage, TInterpolator>::GetTransformParameters() const
{
  const unsigned int num_transforms = transform_.size();
  assert(num_transforms > 0);
...
template <class TImage, class TInterpolator>
void
ImageMosaicVarianceMetric<TImage, TInterpolator>::SetTransformParameters(const params_t & parameters) const
{
  const unsigned int num_transforms = transform_.size();
  assert(num_transforms > 0);
...
template <class TImage, class TInterpolator>
unsigned int
ImageMosaicVarianceMetric<TImage, TInterpolator>::GetNumberOfParameters() const
{
  const unsigned int num_transforms = transform_.size();
...
ImageMosaicVarianceMetric<TImage, TInterpolator>::Initialize()
{
  ...
  for (unsigned int i = 0; i < num_transforms; i++)
  {
    ...
    typename gradient_filter_t::Pointer gradient_filter = gradient_filter_t::New();
    ...
    {
      // oops:
      cerr << "gradient filter threw an exception:" << endl << exception << endl;
      throw exception;
    }
...
template <class TImage, class TInterpolator>
void
ImageMosaicVarianceMetric<TImage, TInterpolator>::GetValueAndDerivative(const params_t & parameters,
                                                                        measure_t &      measure,
                                                                        derivative_t &   derivative) const
{
  WRAP(itk_terminator_t terminator("ImageMosaicVarianceMetric::GetValueAndDerivative"));
  ...
  // mosaic bounding box:
  pnt2d_t              mosaic_min = pnt2d(std::numeric_limits<double>::max(), std::numeric_limits<double>::max());
  ...
  for (itex.GoToBegin(); !itex.IsAtEnd(); ++itex)
  {
    ...
    {
      if (point[0] < min[k][0] || point[0] > max[k][0] || point[1] < min[k][1] || point[1] > max[k][1])
        continue;
      ...
      if (!interpolator_[k]->IsInsideBuffer(pt_k))
        continue;

      // make sure the pixel maps into the mask:
      if (mask_[k].GetPointer() && mask_[k]->GetPixel(index) == 0)
        continue;
    ...
    for (typename std::list<const image_data_t *>::const_iterator i = overlap.begin(); i != overlap.end(); ++i)
    {
      ...
      for (unsigned int j = 0; j < n_params; j++)
      {
        dMu[addr[j]] += (J[0][j] * data.dPdx_ + J[1][j] * data.dPdy_) * normalization_factor;
      }
    }
    ...
    for (typename std::list<const image_data_t *>::const_iterator i = overlap.begin(); i != overlap.end(); ++i)
    {
      ...
      {
        dV[addr[j]] +=
          measure_t(2) * d * (J[0][j] * data.dPdx_ + J[1][j] * data.dPdy_ - dMu[addr[j]]) * normalization_factor;
      }
    }
  ...
  measure_t normalization_factor = measure_t(1) / measure_t(m_NumberOfPixelsCounted);
...
template <class TImage, class TInterpolator>
void
ImageMosaicVarianceMetric<TImage, TInterpolator>::CalcMosaicBBox(point_t &              mosaic_min,
                                                                 point_t &              mosaic_max,
                                                                 ...
                                                                 std::vector<point_t> & max) const
{
  ...
  calc_image_bboxes<image_t>(image_, image_min, image_max);

  std::vector<typename transform_t::ConstPointer> ConstTransforms;
  ConstTransforms.assign(transform_.begin(), transform_.end());

  // mosaic space bounding boxes:
  calc_mosaic_bboxes<point_t, transform_t>(ConstTransforms, image_min, image_max, min, max);
...
ImageMosaicVarianceMetric<TImage, TInterpolator>::variance(measure_t & max_var, measure_t & avg_var) const
{
  max_var = measure_t(0);
  avg_var = measure_t(0);
  ...
  return variance(image_[0]->GetSpacing(), mosaic_min, mosaic_max, max_var, avg_var);
}
...
template <class TImage, class TInterpolator>
typename TImage::Pointer
ImageMosaicVarianceMetric<TImage, TInterpolator>::variance(const typename TImage::SpacingType & mosaic_sp,
                                                           ...
                                                           measure_t &                          avg_var) const
{
  ...
  return variance(mosaic_sp, mosaic_min, mosaic_sz, max_var, avg_var);
}
                                                           ...
                                                           measure_t &                          avg_var) const
{
  ...
  // calculate image bounding boxes in the mosaic space, as well as the
  // mosaic bounding box:
  pnt2d_t              global_min = pnt2d(std::numeric_limits<double>::max(), std::numeric_limits<double>::max());
  ...
  for (itex.GoToBegin(); !itex.IsAtEnd(); ++itex)
  {
    ...
    // calculate the mean and derivative of the mean:
    measure_t Mu = measure_t(0);
    for (std::list<measure_t>::const_iterator i = overlap.begin(); i != overlap.end(); ++i)
    {
      Mu += *i;
    }
    Mu *= normalization_factor;
...
template <class TImage, class TInterpolator>
void
ImageMosaicVarianceMetric<TImage, TInterpolator>::PrintSelf(std::ostream & os, Indent indent) const
{
  // FIXME: write me:

  Superclass::PrintSelf(os, indent);
  os << indent << "MosaicRegion: " << m_MosaicRegion << std::endl
     << indent << "Number of Pixels Counted: " << m_NumberOfPixelsCounted << std::endl;
}
