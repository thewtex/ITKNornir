                      ...
                      const unsigned int num_perms = 4)
{
  ...
  const vec2d_t t[] = {
    vec2d(sx * x, sy * y), vec2d(sx * (x - w), sy * y), vec2d(sx * x, sy * (y - h)), vec2d(sx * (x - w), sy * (y - h))
  };

#ifdef DEBUG_ESTIMATE_DISPLACEMENT
  static const the_text_t fn_save("/tmp/estimate_displacement-");
  static int              COUNTER2 = 0;
  the_text_t suffix = the_text_t::number(COUNTER, 3, '0') + "-" + the_text_t::number(COUNTER2, 3, '0') + "-";
  COUNTER2++;
  ...
  // FIXME:
  {
    ...
    ti->SetOffset(-offset);
    save_rgb<TImage>(fn_save + suffix + "0-init.png", a, b, ti, mask_a, mask_b);
  }
#endif

  std::vector<double> metricArray;
  metricArray.reserve(num_perms);

  std::vector<translate_transform_t::Pointer> transformArray;
  transformArray.reserve(num_perms);

  for (int i = 0; i < (int)num_perms; i++)
  {
    translate_transform_t::Pointer ti = translate_transform_t::New();
    ti->SetOffset(t[i] - offset);

    double area_ratio = overlap_ratio<TImage>(a, b, ti);
    log << i << ": " << setw(3) << int(area_ratio * 100.0) << "% of overlap, ";
    ...
    double metric = eval_metric<metric_t, interpolator_t>(ti,
							  ...
							  mask_b);
#else
    double metric = my_metric<TImage>(area_ratio, a, b, ti, mask_a, mask_b, overlap_min, overlap_max);
    metricArray.push_back(metric);
#endif

    transformArray.push_back(ti);
  }

  for (int i = 0; i < (int)metricArray.size(); i++)
  {
    double                         metric = metricArray[i];
    translate_transform_t::Pointer ti = transformArray[i];
...
#ifdef DEBUG_ESTIMATE_DISPLACEMENT
      // FIXME:
      save_rgb<TImage>(fn_save + suffix + the_text_t::number(i + 1) + "-perm.png", a, b, ti, mask_a, mask_b);
#endif
    }
    else
    {
               ...
               const bool &       consider_zero_displacement)
{
  ...
  unsigned int           total_peaks =
    find_correlation<TImage>(max_list, fi, mi, lp_filter_r, lp_filter_s, overlap_min, overlap_max);
  ...
  for (std::list<local_max_t>::iterator j = max_list.begin(); j != max_list.end(); ++j)
  {
                     ...
                     const mask_t * msk_1)
{
  ...
  double                         metric = match_one_pair<TImage>(log,
  ...
  save_composite(fn_save + suffix + "-a.png", img_0, img_1, identity_transform_t::New(), true);
...
#ifdef DEBUG_REFINE_ONE_POINT
  save_composite(fn_save + suffix + "-b.png", img_0, img_1, translate.GetPointer(), true);
#endif // DEBUG_REFINE_ONE_POINT
                        ...
                        const typename TImage::SpacingType & sp)
{
  ...
  typedef itk::LinearInterpolateImageFunction<TImage, double> interpolator_t;
  typename interpolator_t::Pointer interpolator[] = { interpolator_t::New(), interpolator_t::New() };
  ...
  for (unsigned int y = 0; y < sz[1]; y++)
  {
    ...
    for (unsigned int x = 0; x < sz[0]; x++)
    {
      ...
      // fixed image:
      tile_pt = mosaic_pt;
      if (interpolator[0]->IsInsideBuffer(tile_pt) && pixel_in_mask(mask_0, tile_pt))
      {
      ...
      if (interpolator[1]->IsInsideBuffer(tile_pt) && pixel_in_mask(mask_1, tile_pt))
      {
  ...
  double a[] = { double(area[0]) / max_area, double(area[1]) / max_area };
...
refine_one_point_fft(the_log_t &     log,
                     ...
                     const typename TImage::SpacingType & sp)
{
  if (!refine_one_point_helper<TImage>(
        center, origin, min_overlap, tile_0, mask_0, tile_1, mask_1, t_01, img_0, msk_0, img_1, msk_1, sz, sp))
  {
    return false;
  }

  return refine_one_point_fft<TImage>(log, shift, origin, tile_0, mask_0, img_0, msk_0, img_1, msk_1);
}
                     ...
                     const unsigned int & neighborhood)
{
  ...
  typename TImage::Pointer img[] = { make_image<TImage>(sz), make_image<TImage>(sz) };

  mask_t::Pointer msk[] = { make_image<mask_t>(sz), make_image<mask_t>(sz) };
...
refine_one_point_fft(the_log_t & log,
                     ...
                     const mask_t * msk_1)
{
  typename TImage::SizeType period_sz = img_1->GetLargestPossibleRegion().GetSize();
...
extract(const typename TImage::PointType & origin,
        ...
        const typename TImage::PixelType & bg = itk::NumericTraits<typename TImage::PixelType>::Zero)
{
  ...
  static const typename TMask::PixelType mask_min = itk::NumericTraits<typename TMask::PixelType>::Zero;

  static const typename TMask::PixelType mask_max = itk::NumericTraits<typename TMask::PixelType>::One;
  ...
  for (itex.GoToBegin(); !itex.IsAtEnd(); ++itex)
  {
    ...
    if (interpolator->IsInsideBuffer(tile_pt))
    {
      ...
      {
        tile_val = (typename TImage::PixelType)(interpolator->Evaluate(tile_pt));
        num_pixels++;
      }
    }
...
refine_one_point_helper( // the large images and their masks:
  ...
  mask_t * msk_1)
{
  const typename TImage::SizeType &    sz = img_0->GetLargestPossibleRegion().GetSize();
  const typename TImage::SpacingType & sp = img_0->GetSpacing();

  origin[0] = center[0] - (double(sz[0]) * sp[0]) / 2;
  origin[1] = center[1] - (double(sz[1]) * sp[1]) / 2;

  pnt2d_t corner[] = { origin + vec2d(sz[0] * sp[0], 0),
  ...
  area[0] = extract<TImage, mask_t>(origin, tile_0, mask_0, img_0, msk_0);

  area[1] = extract<TImage, mask_t>(origin, tile_1, mask_1, img_1, msk_1);
  ...
  mask_t * msk_1)
{
  ...
  if (!tiles_intersect<TImage>(tile_0, tile_1, forward_0, forward_1, origin, sz, sp))
  {
    return false;
  }
                     ...
                     mask_t * msk_1)
{
  pnt2d_t origin;
  if (!refine_one_point_helper<TImage>(
        tile_0, mask_0, tile_1, mask_1, center, origin, min_overlap, img_0, msk_0, img_1, msk_1))
  {
    return false;
  }
                     ...
                     mask_t * msk_1)
{
  ...
  return refine_one_point_fft<TImage>(log, shift, origin, img_0_large, msk_0_large, img_0, msk_0, img_1, msk_1);
}
                     ...
                     mask_t * msk_1)
{
  ...
  return refine_one_point_fft<TImage>(log, shift, img_0, msk_0, img_1, msk_1);
}
                ...
                const double &     max_step)
{
  ...
  optimizer_observer_t<optimizer_t>::Pointer observer = optimizer_observer_t<optimizer_t>::New();
  observer->log_ = &log;
  ...
  typedef itk::NormalizedCorrelationImageToImageMetric<TImage, TImage> metric_t;
  ...
  double metric_before = eval_metric<metric_t, interpolator_t>(t01, fi, mi, fi_mask, mi_mask);
  ...
  {
    log << "image registration threw an exception: " << endl << exception.what() << endl;
  }
  ...
  log << "BEFORE: " << metric_before << endl << "AFTER:  " << metric_after << endl;
                       ...
                       const mask_t * msk_1)
{
  ...
  double metric = refine_one_pair<translate_transform_t, TImage>(translate,
                                                                 tile_0,
                                                                 mask_0,
                                                                 img_1,
                                                                 msk_1,
  ...
  the_text_t              suffix = the_text_t::number(COUNTER, 3, '0');
  {
    ...
    t->SetOffset(-offset);
    save_composite(fn_save + suffix + "-a.png", tile_0, img_1, t.GetPointer(), true);
  }
#endif // DEBUG_REFINE_ONE_POINT
...
#ifdef DEBUG_REFINE_ONE_POINT
  save_composite(fn_save + suffix + "-b.png", tile_0, img_1, translate.GetPointer(), true);
#endif // DEBUG_REFINE_ONE_POINT
                       ...
                       const typename TImage::SpacingType & sp)
{
  ...
  the_text_t              suffix = the_text_t::number(COUNTER, 3, '0');
  save_composite(
    fn_save + suffix + "-a.png", img_0.GetPointer(), img_1.GetPointer(), identity_transform_t::New(), true);
#endif // DEBUG_REFINE_ONE_POINT
...
#ifdef DEBUG_REFINE_ONE_POINT
  save_composite(fn_save + suffix + "-b.png", img_0.GetPointer(), img_1.GetPointer(), translate.GetPointer(), true);
#endif // DEBUG_REFINE_ONE_POINT
                       ...
                       const unsigned int & neighborhood)
{
  ...
  return refine_one_point_nofft<TImage>(
    shift, center, origin, min_overlap, tile_0, mask_0, tile_1, mask_1, t_01, img[0], msk[0], img[1], msk[1], sz, sp);
}
