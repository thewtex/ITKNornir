            ...
            const itk::TransformBase ** transform)
{
  ...
  so << "format_version_number: " << 1 << endl
     << "number_of_images: " << num_images << endl
     << "pixel_spacing: " << pixel_spacing << endl
     << "use_std_mask: " << use_std_mask << endl;

  for (unsigned int i = 0; i < num_images; i++)
  {
    // Find just the filename to save out to the mosaic.
    so << "image:" << endl << image[i] << endl;
...
the_thread_pool_t * save_mosaic::_pthread_pool;
