...
#define wc barycentric_coords[2]
  ...
  if (wb < 0 && wb > -EPSILON)
    wb = 0;
  if (wb > 1 && (wb - 1) < EPSILON)
    wb = 1;
  if (wc < 0 && wc > -EPSILON)
    wc = 0;
  if (wc > 1 && (wc - 1) < EPSILON)
    wc = 1;

  wa = 1.0 - (wb + wc);
  if (wa < 0 && wa > -EPSILON)
    wa = 0;
  if (wa > 1 && (wa - 1) < EPSILON)
    wa = 1;
...
//
bool
triangle_t::xy_intersect(const vertex_t * v_arr, const pnt2d_t & xy, pnt2d_t & uv) const
{
#define wa barycentric_coords[0]
#define wb barycentric_coords[1]
#define wc barycentric_coords[2]
  ...
  if (WARPING && (fabs(wa) < THRESHOLD || fabs(wb) < THRESHOLD || fabs(wc) < THRESHOLD))
  {
    uv[0] = 0.0;
    uv[1] = 0.0;
  }
...
//
bool
triangle_t::uv_intersect(const vertex_t * v_arr, const pnt2d_t & uv, pnt2d_t & xy) const
{
#define wa barycentric_coords[0]
#define wb barycentric_coords[1]
#define wc barycentric_coords[2]
...
the_acceleration_grid_t::xy_cell(const pnt2d_t & xy) const
{
  ...
  {
    double c = a * double(cols_);
    double r = b * double(rows_);

    unsigned int col = c >= cols_ ? cols_ - 1 : c;
    unsigned int row = r >= rows_ ? rows_ - 1 : r;
...
the_acceleration_grid_t::xy_triangle(const pnt2d_t & xy, pnt2d_t & uv) const
{
  ...
  {
    // shortcuts:
    const std::vector<unsigned int> & cell = xy_[cell_id];
    const vertex_t *                  v_arr = &(mesh_[0]);

    // check each candidate triangle for intersection:
    for (int iter = 0; iter < (int)cell.size(); iter++)
    {
      unsigned int       iTri = cell[iter];
      const triangle_t & tri = tri_[iTri];
...
the_acceleration_grid_t::uv_triangle(const pnt2d_t & uv, pnt2d_t & xy) const
{
  ...
  {
    // shortcuts:
    const std::vector<unsigned int> & cell = uv_[cell_id];
    const vertex_t *                  v_arr = &(mesh_[0]);

    // check each candidate triangle for intersection:
    for (unsigned int iter = 0; iter < cell.size(); iter++)
    {
      unsigned int       iTri = cell[iter];
      const triangle_t & tri = tri_[iTri];
...
//
void
the_acceleration_grid_t::resize(unsigned int rows, unsigned int cols)
{
  // reset the grid bounding box:
  xy_min_[0] = std::numeric_limits<double>::max();
  xy_min_[1] = xy_min_[0];
...
the_acceleration_grid_t::rebuild()
{
  ...
  // reset to empty grid:
  xy_.assign(xy_.size(), std::vector<unsigned int>());
  uv_.assign(uv_.size(), std::vector<unsigned int>());
  ...
  double & v)
{
  ...
  double A_inv[][2] = { { -dy * det_A_inv, dx * det_A_inv }, { -by * det_A_inv, bx * det_A_inv } };
                        ...
                        const pnt2d_t & C)
{
  ...
  double lines_b[][4] = { { A[0], A[1], B[0] - A[0], B[1] - A[1] },
  ...
  double * pwc)
{
  ...
  double         a[] = { (min[0] - grid_min[0]) / gw, (max[0] - grid_min[0]) / gw };

  unsigned int c[] = { std::min(cols - 1, (unsigned int)(floor(a[0] * double(cols)))),
                       std::min(cols - 1, (unsigned int)(floor(a[1] * double(cols)))) };

  const double & gh = grid_ext[1];
  double         b[] = { (min[1] - grid_min[1]) / gh, (max[1] - grid_min[1]) / gh };

  unsigned int r[] = { std::min(rows - 1, (unsigned int)(floor(b[0] * double(rows)))),
  ...
  for (unsigned int row = r[0]; row <= r[1]; row++)
  {
    for (unsigned int col = c[0]; col <= c[1]; col++)
    {
      ...
      std::vector<unsigned int> & cell = grid[i];
      ...
      if (inside_bbox(min, max, A) || inside_bbox(min, max, B) || inside_bbox(min, max, C) ||
      ...
      {
        // James A: Modifying this to be a bit more selective about which triangles are added to which grid cells.  If a
        // triangle only shares an edge with the top or right cell wall we do not add it
        unsigned int numVertsInsideBBox = 0;
        numVertsInsideBBox += inside_bbox(min, max, A);
        numVertsInsideBBox += inside_bbox(min, max, B);
        numVertsInsideBBox += inside_bbox(min, max, C);

        // If all three verts are inside the box then this it is more likely this triangle will match a test than a
        // triangle with only one or two verts inside the box.  Put it at the front of the line
        if (numVertsInsideBBox == 3)
          cell.insert(cell.begin(), tri_id);
        else
...
//
bool
the_base_triangle_transform_t::transform(const pnt2d_t & xy, pnt2d_t & uv) const
{
...
//
bool
the_base_triangle_transform_t::transform_inv(const pnt2d_t & uv, pnt2d_t & xy) const
{
...
//
bool
the_base_triangle_transform_t::jacobian(const pnt2d_t & P, unsigned int * idx, double * jac) const
{
  pnt2d_t      uv;
                            ...
                            const std::vector<pnt2d_t> & xy)
{
  ...
  grid_.resize(rows_ * GRID_DENSITY, cols_ * GRID_DENSITY);
                            ...
                            unsigned int                 accel_grid_cols)
{
  ...
  grid_.resize(accel_grid_rows, accel_grid_cols);
...
//
bool
the_mesh_transform_t::insert_point(const pnt2d_t & uv, const pnt2d_t & xy, const bool delay_setup)
{
  vertex_t vx;
  vx.uv_ = uv;
  vx.xy_ = xy;
...
//
static int
Triangulate(int num_vertices, TVertex * vertex, TTriangle * tri, const int max_triangles, int & num_triangles)
{
  ...
  {
    if (tri[i].p1 >= num_vertices || tri[i].p2 >= num_vertices || tri[i].p3 >= num_vertices)
    {
...
the_mesh_transform_t::setup_mesh()
{
  ...
  // sort the vertices on the x-coordinate, in ascending order:
  std::sort(vertex_vec.begin(), vertex_vec.begin() + num_vertices, &TVertexCompare);
  ...
  int num_triangles = 0;
  int error = ::Triangulate(num_vertices, vertices, triangles, max_triangles, num_triangles);
  if (error != 0)
  {
