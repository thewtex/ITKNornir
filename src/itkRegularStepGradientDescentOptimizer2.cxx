...
RegularStepGradientDescentOptimizer2::StartOptimization()
{
  ...
  {
    itkExceptionMacro(<< "The size of Scales is " << scalesSize
  ...
  m_BestValue = this->m_Maximize ? -std::numeric_limits<MeasureType>::max() : std::numeric_limits<MeasureType>::max();
  m_Value = m_BestValue;
...
RegularStepGradientDescentOptimizer2::ResumeOptimization()
{
  ...
  while (!m_Stop)
  {
    ...
    if ((this->m_Maximize && m_Value < m_PreviousValue) || (!this->m_Maximize && m_Value > m_PreviousValue))
    {
      // relax the step size:
      ...
      if (m_BackTracking)
      {
        ...
        *log_ << m_Value << " vs " << m_BestValue
              << " -- relaxing and backtracking, new step length: " << m_CurrentStepLength << std::endl;
      ...
      {
        // FIXME:
        *log_ << m_Value << " vs " << m_PreviousValue << " -- relaxing, new step length: " << m_CurrentStepLength
              << std::endl;
      }
    ...
    else
    {
      ...
      if (successful_steps % m_PickUpPaceSteps == 0)
      {
        ...
        m_CurrentStepLength = std::min(m_MaximumStepLength, m_CurrentStepLength / m_RelaxationFactor);

        // FIXME:
        *log_ << successful_steps << " successful steps -- increasing pace, new step length: " << m_CurrentStepLength
              << std::endl;
      }
    }
    ...
    if ((this->m_Maximize && m_Value > m_BestValue) || (!this->m_Maximize && m_Value < m_BestValue))
    {
      m_BestValue = m_Value;
...
//
void
RegularStepGradientDescentOptimizer2::StepAlongGradient(double factor, const DerivativeType & transformedGradient)
{
...
//
void
RegularStepGradientDescentOptimizer2::PrintSelf(std::ostream & os, Indent indent) const
{
